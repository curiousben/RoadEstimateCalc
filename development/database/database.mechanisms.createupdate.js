...
function pageErrors() {
	var errorStorageString = localStorage.getItem('ErrorLog');
	if (errorStorageString == '' || errorStorageString == null) {
		...
		$('#Errors').html('You are seeing this window because there are errors on the data record that needs to be fixed before you are able to create/update a record. These errors can be found and fixed by following the red dot.');
	};
};
...
function DataArrayObject () {
	//The Data Record with pure data points and their respective keys in local storage
	DataArray={};
		for (var i = localStorage.length - 1; i >= 0; i--) {
			var Key=localStorage.key(i);
			if (Key != 'ImportEstimator' || Key != 'ImportUPC' || Key != 'ImportRoadNumber' || Key != 'ErrorLog') {	
